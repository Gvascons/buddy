...
def test_cell_style_tag_is_not_misparsed():
    """Old grid-cell style tags like [POINT:H6:label] should NOT be
    accepted as pixel POINTs — they're gone from the prompt, and if
    Claude emits one we want it treated as 'no point' rather than
    interpreted as bogus coordinates.
    """
    result = parse_point("this is a ghost cell. [POINT:H6:foo]")
    # POINT_REGEX is pixel-only now, so this shouldn't match at all.
    # The bracketed text stays in the response, no coord is set.
    assert not result.has_coordinate
    assert "[POINT:H6:foo]" in result.spoken_text
